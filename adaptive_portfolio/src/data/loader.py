...
class MoexDataLoader:
    ...
    def _cache_path(self, key: str) -> Path:
        ...
        Под Windows путь ограничен MAX_PATH=260 символами. Если в выборке много тикеров
        (40+), список в имени легко выходит за этот лимит — open() падает с FileNotFoundError.
        Чтобы этого избежать, длинные ключи хешируются.
        """
        safe = key.replace(":", "_").replace("/", "_").replace("\\", "_")
        if len(safe) > 80:
            digest = hashlib.sha1(safe.encode("utf-8")).hexdigest()[:16]
            # Оставляем короткий префикс для читаемости («prices_» или «imoex_» и т.д.).
            prefix_end = safe.find("_")
            prefix = safe[:prefix_end] if prefix_end > 0 else "data"
    ...
    def load(
        ...
        if prices_df is None and not force_synthetic:
            try:
                ...
                # write cache (падение записи не должно ломать загрузку)
                try:
                    ...
                    logger.warning(
                        "Не удалось записать кэш в %s: %s. Продолжаю без кэша.",
                        cache_path, exc,
                    )
